...
class S5002Parser:
    ...
    # Namespace do e-Social (tentar diferentes versões)
    NS_VERSIONS = [
        {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_03_00'},
        {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_02_00'},
        {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_01_00'}
    ]
    
    # Usar a primeira versão como padrão
    NS = NS_VERSIONS[0]
    ...
    def __init__(self, xml_path: str):
        ...
        self.tree = None
        self.root = None
        self.ns = self.NS  # Namespace a ser usado para este arquivo específico
    ...
    def parse(self) -> List[ComprovanteRendimentos]:
        ...
        try:
            ...
            # Determinar o namespace correto para este arquivo
    ...
    def _detect_namespace(self):
        """Detecta o namespace correto para o arquivo XML"""
        if self.root is not None:
            root_tag = self.root.tag
            
            # Verificar se o namespace está no próprio elemento raiz
            for ns_dict in self.NS_VERSIONS:
                ns_url = list(ns_dict.values())[0]
                if ns_url in root_tag:
                    return ns_dict
        
        # Se não encontrou no elemento raiz, tentar o namespace padrão
        return self.NS
    ...
    def _parse_trabalhador(self, ide_trab) -> Optional[ComprovanteRendimentos]:
        ...
        try:
            ...
            if self.root is not None:
                ...
                if per_apur is not None and per_apur.text:
                    ano = per_apur.text[:4]
            ...
            if ide_empregador is not None:
                ...
                if nr_insc is not None and nr_insc.text is not None:
...
class PDFGenerator:
    ...
    def gerar_pdf(self, comprovante: ComprovanteRendimentos, output_path: str):
        ...
        try:
            ...
            max_iteracoes = 5
            total_pages_real = 1  # Inicializar a variável
...
def consolidar_comprovantes(comprovantes: List[ComprovanteRendimentos]) -> Optional[ComprovanteRendimentos]:
...
def extrair_cpf_xml(xml_path: str) -> Optional[str]:
    ...
    try:
        ...
        # Tentar diferentes versões de namespace
        namespaces = [
            {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_03_00'},
            {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_02_00'},
            {'esocial': 'http://www.esocial.gov.br/schema/evt/evtIrrfBenef/v_S_01_01_00'}
        ...
        # Tentar encontrar CPF com cada namespace
            ...
            if evento is not None:
                for child in evento:
                    if 'eSocial' in child.tag:
                        # Tentar encontrar CPF com cada namespace no XML interno
                        for ns in namespaces:
...
def processar_xml(args: Tuple[str, str, str, Optional[str], DadosComplementares]) -> Tuple[int, int]:
    ...
    try:
        ...
        sucesso = 0
        erros = 0
        cpf = "desconhecido"  # Initialize cpf to avoid unbound variable error
        ...
        for comprovante in comprovantes:
            try:
                ...
                for pensao in comprovante.pensoes_alimenticias:
                    # The PensaoAlimenticia class uses cpf_beneficiario, not cpf_dep
                    # and doesn't have a nome_dep attribute, so we skip this
                    pass
...
def processar_xmls(input_dir: str, output_dir: str, ano: Optional[str] = None, workers: int = 4, 
    ...
    # Ano padrão
    if not ano:
        ano = str(datetime.now().year - 1)
    
    # Carregar dados complementares
    dados_compl = DadosComplementares(csv_path, csv_dependentes, csv_entidades)
    
    # Listar arquivos XML
    xml_files = list(Path(input_dir).glob('*.xml'))
    
    if not xml_files:
        return 0, 0, "Nenhum arquivo XML encontrado"
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for cpf, xmls_do_cpf in grupos_cpf.items():
            future = executor.submit(
                ...
                (xmls_do_cpf, output_dir, ano, csv_path, dados_compl)
            )
    ...
    return total_sucesso, total_erros, f"Processamento concluído: {total_sucesso} sucessos, {total_erros} erros"
...
def main():
    ...
    fim = datetime.now()
    duracao = fim - inicio
    logger.info(f"Processamento concluído em {duracao.total_seconds():.2f} segundos")
    logger.info(f"Total de sucessos: {total_sucesso}")
